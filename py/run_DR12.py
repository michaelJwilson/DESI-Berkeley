...
'''
To run interactively on NERSC:
i)   salloc -N 1 -q debug -L SCRATCH or 'interactive' alias.
ii)  nkit 3.6
iii) module load python
iii) srun -N 1 python run_DR12.py

## To download the data and corresponding random BOSS dataset (Two files for North and South)                                                                                                                                   
...
## To download the simulated BOSS datasets                                                                                                                                                                                      
...
base_dir     = '/scratch1/scratchdirs/mjwilson/cov/DR12/'
output_path  = '/scratch1/scratchdirs/mjwilson/cov/DR12/'
...
def read_sim_data(filename, P0=10000.):
    ...
    sim_cat['WEIGHT_FKP']    = 1.0
    ## sim_cat['WEIGHT_FKP'] = 1.0/(1. + P0*sim_cat['NZ']);

    return sim_cat

def read_sim_ran(filename, P0=10000.):
    ''' Read the simulation catalogues from CSV. '''
    ran_cat = nbodykit.lab.CSVCatalog(os.path.join(base_dir, filename),\
                                      names=['RA', 'DEC', 'Z', 'Weight'])  ## names=['RA', 'DEC', 'Z', 'NZ', 'dummy1', 'veto', 'Weight'])
    ...
    ran_cat['WEIGHT_FKP']    = 1.0 
    ## ran_cat['WEIGHT_FKP'] = 1.0/(1. + P0*ran_cat['NZ']);
...
def calc_pk(cosmo, data, random):
    ...
    data['Position']   = nbodykit.transform.SkyToCartesian(  data['RA'],   data['DEC'],   data['Z'],   cosmo=cosmo)
    random['Position'] = nbodykit.transform.SkyToCartesian(random['RA'], random['DEC'], random['Z'], cosmo=cosmo)
...
def process_data(cosmo, tag):
    # Read the BOSS data                                                                                                                                                                                                        
    filename = 'data/galaxy_DR12v5_LOWZ_%s.fits' % tag
    galaxies =  read_data(filename)

    # Read the random catalogues for calibration                                                                                                                                                                                
    filename = 'data/random0_DR12v5_LOWZ_%s.fits' % tag
...
def cmp_pk_with_error(pk1, pk2):
    ...
    plt.errorbar(pk1['k'], pk1['k']*pk1['pk'], yerr=pk1['k']*np.sqrt(np.diagonal(pk1['cov'])), marker='.', linestyle = 'None', label=pk1['label'])
    plt.errorbar(pk2['k'], pk2['k']*pk2['pk'], yerr=pk2['k']*np.sqrt(np.diagonal(pk2['cov'])), marker='.', linestyle = 'None', label=pk2['label'])
...
def cmp_ratio_with_error(pk1, pk2, models=[]):
    ...
    plt.errorbar(pk1['k'], pk1['ratio'], yerr=pk1['ratio_err'], marker='.', linestyle = 'None', label=pk1['label'])
    plt.errorbar(pk2['k'], pk2['ratio'], yerr=pk2['ratio_err'], marker='.', linestyle = 'None', label=pk2['label'])
...
def calc_cov(tag, N):
    ...
    for i in range(1, N):
        pk_file = output_path + "/pk/qpm/pk_%s_%d.pickle" % (tag, i)
...
def process_sims(cosmo, tag, N):
    ...
    filename    = 'mocks/qpm/mock_random_DR12_LOWZ_S_50x1.rdzw' ## NOTE:  ra, dec, z, weight.  
    ## filename = 'mocks/patchy/Patchy-Mocks-Randoms-DR12%s-COMPSAM_V6C_x50.dat' % tag
    ...
    for i in range(1, N):
        ...
        ## Only calculate the power spectra that we don't have on disk.                                                                                                                                                        
        if not os.path.isfile(pk_file):
            filename    = base_dir + '/mocks/qpm/mock_galaxy_DR12_LOWZ_S_QPM_%0.4d.rdzw' % i
            ## filename = base_dir + '/mocks/patchy/Patchy-Mocks-DR12%s-COMPSAM_V6C/Patchy-Mocks-DR12%s-COMPSAM_V6C_%0.4d.dat' % (tag, tag, i)
...
def get_oscillation(krange, Pk_class, Pk_without_BAO):
    ...
    result  = op.minimize(calc_chi2, start, args=( [{ 'k': krange, 'pk': Pk_class(krange), 'cov_inv': cov_inv }], {'noBAO': Pk_without_BAO }, get_smooth_model))
...
def inspect_chain(list_of_samplers, labels=[]):
    ...
    try:
        fig = corner.corner(mergedsamples, quantiles=[0.16, 0.5, 0.84], plot_density=False, show_titles=True, title_fmt=".3f", labels=labels)
    ...
    fig.savefig("%s_timeseries.pdf" % output_path)
...
def runMCMC(start, data, templates):
    ...
    for jj in range(0, Nchains):
        ...
        list_of_samplers.append(emcee.EnsembleSampler(nwalkers=nwalkers, dim=dim, lnpostfn=get_loglike, args=(data, templates, get_shifted_model)))
...
if __name__ == '__main__':
    ...
    pk_file = "/scratch1/scratchdirs/mjwilson/cov/DR12/pk/data/pk_South.pickle"
    ...
    '''
    ## and for the North.
    pk_file = "/scratch1/scratchdirs/mjwilson/cov/DR12/pk/data/pk_North.pickle"
    ...
    var_south  = np.copy(np.diag(cov_south))
    ## var_north = np.diag(cov_north)

    cor_south  = cov_south / np.sqrt(var_south[:, None]) 
    cor_south /= np.sqrt(var_south[None, :])

    ## cor_north  = cov_north / np.sqrt(var_north[:, None])
    ## cor_north /= np.sqrt(var_north[None, :])
    
    plt.matshow(cor_south)
    
    pl.savefig('qpmcor_south.pdf')
    ...
    boss_data = [{ 'label': 'BOSS south',
                   'k': pk_south['k'][(pk_south['k'] < kmax)], 
                   'pk': P_south[(pk_south['k'] < kmax)],\
                   'cov': cov_south, 
                   'cov_inv': cov_south_inv},\
                 { 'label': 'BOSS north',
                   'k': pk_north['k'][(pk_north['k'] < kmax)], 
                   'pk': P_north[(pk_north['k'] < kmax)],\
                   'cov': cov_north, 
                   'cov_inv': cov_north_inv }]
    ...
    result         = op.minimize(calc_chi2, start, args=(boss_data, { 'noBAO': Pk_without_BAO, 'os_model': os_model }, get_shifted_model))
    ...
    if per is not None:
        ...
        print("There's a problem with the MCMC chain.")
    '''
